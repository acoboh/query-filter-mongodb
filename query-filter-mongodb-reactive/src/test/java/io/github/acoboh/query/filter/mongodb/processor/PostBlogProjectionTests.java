...
class PostBlogProjectionTests {
	...
	void queryById() {
		...
		StepVerifier.create(list).expectNext(DOC1_DTO).verifyComplete();
		...
		StepVerifier.create(list).expectNext(DOC2_DTO).verifyComplete();
	...
	void testSortProjectBase() {
		...
		var list = qf.executeAggregateAndProject(PostBlogSimplifiedDTO.class);
		StepVerifier.create(list).expectNext(DOC1_DTO).expectNext(DOC2_DTO).verifyComplete();

		qf = qfProcessor.newQueryFilter("sort=-id", QFParamType.RHS_COLON);
		assertThat(qf).isNotNull();

		list = qf.executeAggregateAndProject(PostBlogSimplifiedDTO.class);
		StepVerifier.create(list).expectNext(DOC2_DTO).expectNext(DOC1_DTO).verifyComplete();
