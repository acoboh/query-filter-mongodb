package io.github.acoboh.query.filter.mongodb.converters;
...
class ConverterTests {
	...
	private static class AnnotationClass {
		...
		public static void paramQueryFilterBlogDef(
				@QFParam(BasicDocumentFilterDef.class) QueryFilter<BasicDocument> filter) {
	...
	@Test
	@DisplayName("1. Test conversion filter to query filter BasicDocumentFilterDef")
	void testConversionFilterToQueryFilter() throws NoSuchMethodException, SecurityException {
		...
		ResolvableType type = ResolvableType.forClassWithGenerics(QueryFilter.class, BasicDocument.class);
		...
		assertThat(queryFilter.getEntityClass()).isEqualTo(BasicDocument.class);
		assertThat(queryFilter.getPredicateClass()).isEqualTo(BasicDocumentFilterDef.class);
